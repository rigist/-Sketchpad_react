...
///////////////////////////////////////
const notesInit = [
  { id: 1, text: "Первая запись" },
  { id: 2, text: "Вторая запись" },
  { id: 3, text: "Третья запись" }
];
//////////////////////////////////////////   APP
function App() {
  //*******RD***********************
  ...
  const result = notes.map((note) => {
    return (
      ...
      >
        {trimText(note.text)}
        <button
          style={{ margin: 5, position: "absolute", right: 0, bottom: -5 }}
  ...
  function getValue() {
    ...
    if (editId) {
      return notes.reduce(
        (res, note) => (note.id === editId ? note.text : res),
        ""
      );
    } else {
      return obj.text;
  ...
  function search() {
    ...
    let searchNotes = notes.filter((note) => {
      if (note.text.toLowerCase().indexOf(valueInput.toLowerCase()) >= 0) {
        return note;
      }
    });
  ...
  return (
    <div style={{ backgroundColor: "green", margin: 3, padding: 2 }}>
      ...
      <div id="header" style={{ display: "flex", flexDirection: "row" }}>
        <ul>{result}</ul>
        <textarea
          style={{ width: 574, height: 199, margin: 10, borderRadius: 10 }}
          value={getValue()}
          onChange={
            (event) =>
              changeItem(event) /* event => setValueArea(event.target.value) */
          }
        />
      </div>

      <button style={{ position: "absolute", left: 500 }} onClick={saveItem}>
        Сохранить
      </button>
      {/* RD */}
      {/* <div>{cash}</div> */}
